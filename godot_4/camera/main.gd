...
func test_triangle_hit( P1, P2, P3 ):
	...
	ray = ($Ray.translation - source).normalized()
	
	var surface_normal = (P2 - P1).cross(P3 - P1)
	a = -surface_normal.dot( P1 )

	if surface_normal.dot(ray) !=0 :
		t =  -( a + surface_normal.dot( source ) )
		t /= surface_normal.dot(ray)
		if t >= 0:
			var surface_hit = source + ray * t

...
func get_bezier_1(ofs, A, B): # equivalent to linear interpolation (see docs: lerp)
	var t = float(ofs) / steps
	return (1 - t) * A + t * B


func get_bezier_2(ofs, A, B, C): # quadratic
	var t = float(ofs) / steps
	var t_1 = 1 - t
	return pow(t_1, 2) * A + 2 * t * t_1 * B + pow(t, 2) * C


func get_bezier_3(ofs, A, cA, B, cB): # cubic
	var t = float(ofs) / steps
	var t_1 = 1 - t
